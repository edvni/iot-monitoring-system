...
static const unsigned char* private_key = (unsigned char*)PRIVATE_KEY; // Used for signing the JWT
...
// Creates the JWT payload by formatting the payload string with the provided values like iss (issuer), sub (subject), iat (issued at) and exp (expiration time)
static int generate_payload(unsigned char* output, size_t size, const unsigned char* input, long int seconds) {
    return snprintf((char*)output, size, (char*)input, ISS, SUB, (long int)seconds, (long int)seconds + TOKEN_EXPIRATION_TIME_IN_SECONDS);
}
...
static void generate_jwt(long int time_since_epoch) {
    ...
    snprintf((char*)header, 256, (char*)header_format, KEY_ID); // Create the header JSON

    unsigned char payload[512];
    generate_payload(payload, 512, payload_format, time_since_epoch); // Create the payload JSON
