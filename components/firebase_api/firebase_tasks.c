...
// Function to send batch sensor data with retries
esp_err_t firebase_send_batch_data_with_retries(const char *collection, const char *batch_json, int max_retries) {
    ...
    for (int i = 0; i < max_retries; i++) {
        ...
        ret = firebase_send_data_safe(collection, NULL, batch_json);
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "Succesfully sent batch data to Cloud Firestore");
            return ESP_OK;
        }

        ESP_LOGI(TAG, "Failed to send batch data, retrying (%d/%d)", i + 1, max_retries);
        vTaskDelay(pdMS_TO_TICKS(1000)); // Wait before retrying
    }

    ESP_LOGE(TAG, "All batch data send retries failed");
    return ret;
}

// Function to send multimple sensor readings in a single document
esp_err_t firebase_send_multiple_readings(const char *tag_id, const float *temperatures, const float *humidities, const time_t *timestamps, int count, int max_retries) {
    // Create a buffer for the batch JSON data
    // Format: { "tag_id": "AA:BB:CC:DD:EE:FF", "readings": [ {...}, {...}, ... ] } ... ] }
    const size_t buffer_size = 4096;
    char *batch_json = malloc(buffer_size);

    if (batch_json == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for batch JSON data");
        return ESP_ERR_NO_MEM;
    }

    // Prepare the batch JSON header
    int offset = snprintf(batch_json, buffer_size, "{\"tag_id\": \"%s\", \"readings\": [", tag_id);

    // Add each reading as a JSON object in the array
    for (int i = 0; i < count && offset < buffer_size - 100; i++) {
        // Add comma if not the first reading
        if (i > 0) {
            offset += snprintf(batch_json + offset, buffer_size - offset, ",");
        }

        // Add the reading as a JSON object
        offset += snprintf(batch_json + offset, buffer_size - offset, 
            "{\"temperature\": %.2f, \"humidity\": %.2f, \"timestamp\": %ld}", 
            temperatures[i], humidities[i], (long)timestamps[i]);
    }

    // Close the JSON array and document
    offset += snprintf(batch_json + offset, buffer_size - offset, "]}");

    // Send the batch data
    esp_err_t result = firebase_send_batch_data_with_retries("sensor_batches", batch_json, max_retries);

    free(batch_json); // Free the allocated memory for batch JSON data
